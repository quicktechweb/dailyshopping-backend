    ...
    for (let i = 0; i < quantity; i++) {
      ...
      const purchase = new CouponPurchase({
        ...
        couponlimit: userData.couponlimit,
          selectedSize: userData.selectedSize || null,    
  selectedColor: userData.selectedColor || null, 
...
export const getsMyPurchases = async (req, res) => {
  try {
    ...
    // ✅ trim + normalize so stray spaces/empty strings don't cause bad matches
    ...
    // ✅ only include conditions for fields that actually have a value,
    // so an empty string never matches a field that happens to be "" or missing
    const orConditions = [];
    if (phone) orConditions.push({ userRegPhone: phone });
    if (email) orConditions.push({ useremail: email.toLowerCase() });
...
// --- DELETE MULTIPLE PURCHASES (bulk / whole round delete) ---
export const deleteMultiplePurchases = async (req, res) => {
  try {
    const { ids } = req.body;
    ...
    const result = await CouponPurchase.deleteMany({ _id: { $in: ids } });
