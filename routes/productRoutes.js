  ...
  try {
    ...
    for (const imgUrl of images) {
      ...
      try {
        ...
        // 2️⃣ Convert to JPG (Sharp handles AVIF/WEBP/PNG)
        await sharp(originalTemp)
          .jpeg({ quality: 90 })
          .toFile(convertedTemp);

        // 3️⃣ Generate hash from converted image
...
// ✅ Seller-এর নিজের product search + pagination
router.get("/seller-search/:sellerId", async (req, res) => {
  try {
    const { sellerId } = req.params;
    ...
    if (q.trim()) {
      ...
      filter.$and = words.map((word) => ({
        $or: [
          ...
          { subcategoryName: { $regex: word, $options: "i" } },
          { childcategoryName: { $regex: word, $options: "i" } },
...
// ✅ শুধু pending product গুলো আনার জন্য (admin approval list)
router.get("/pending-products", async (req, res) => {
  try {
    ...
    const filter = { uploadstatus: "pending" };
...
// ✅ Product-wise Discount আপডেট করার route
router.put("/:id/productwise-discount", async (req, res) => {
  try {
    const { productwiseDiscount } = req.body;

    if (productwiseDiscount == null) {
      return res.status(400).json({ success: false, message: "productwiseDiscount required" });
    }

    const updated = await Product.findByIdAndUpdate(
      req.params.id,
      { $set: { productwiseDiscount: Number(productwiseDiscount) } },
      { new: true }
    );
    ...
    res.json({ success: true, message: "Discount updated", product: updated });
  } catch (err) {
...
router.get("/", async (req, res) => {
  try {
    ...
    const cursor = Product.find({ uploadstatus: "approved" })
      .sort({ createdAt: -1 })
      .lean()
      .cursor();
...
// });
  ...
  // 🔍 ADMIN PRODUCT SEARCH — fast, paginated, no coupon-stats overhead
router.get("/admin-search", async (req, res) => {
  try {
    ...
    if (q.trim()) {
      ...
      // প্রতিটা শব্দ title/category/subcategory/childcategory/brand-এর যেকোনো একটাতে মিললেই সেই product ম্যাচ করবে
      filter.$and = words.map((word) => ({
        $or: [
          { title: { $regex: word, $options: "i" } },
          { categoryName: { $regex: word, $options: "i" } },
...
// ============================
// GET category-wise promo list (jegula already set kora)
// ============================
router.get("/promo/category-list", async (req, res) => {
  try {
    const promos = await Product.aggregate([
      { $match: { promoCode: { $exists: true, $ne: "" } } },
      {
        $group: {
          _id: "$categoryName",
          ...
          promoStartDate: { $first: "$promoStartDate" },
          promoEndDate: { $first: "$promoEndDate" },
          productCount: { $sum: 1 },
        },
      },
    ]);
...
// ============================
// DELETE category-wise promo (fields unset kore dibe)
// ============================
router.put("/updatepromo/category/delete", async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: `Promo removed for ${categoryName}`,
